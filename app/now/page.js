...
import { asyncGetNowPlaying } from '@/store/Actions/nowPlayingMoviesAction';
import { changNowPlayingPage } from '@/store/Reducers/nowPlayingMoviesReducer';
const page = () => {
    const dispatch = useDispatch()
    const {movies,page, errors} = useSelector(state =>state.NowPlayingMoviesReducer)

    console.log("ksjfkldjf" , movies)

    useEffect(()=>{
        dispatch(asyncGetNowPlaying())
    },[page])

    

    const funky = (e)=>{
        dispatch(changNowPlayingPage(e))
  ...
  return (
    <>
     ...
     <div className=" min-h-[100vh] mx-auto sm:w-[80vw] mt-5">
          <div className=" text-black text-3xl font-bold p-10">Now Playing</div>
